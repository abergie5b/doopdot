...
namespace image
{
    ...
    Image::Image(std::string _filename, std::string type)
    {
        ...
        if (image_type == ImageType::JPEG)
        {
            ...
            _headers.max_grayscale = 255; // default
            _headers.pixel_size = img.m_pixelSize;
            data = img.getData();
            headers = _headers;
            if (img.m_colourSpace == 1) // not sure
            {
                is_rgb = true;
            }
        }
    ...
    int Image::writeas(std::string path, std::string type)
    {
        ...
        else if (_image_type == ImageType::PGM)
        {
    ...
    {
        data = algos::convert_to_grayscale(rgb_data);
        image_type = ImageType::PGM; // default
        reset_headers();        
    }
