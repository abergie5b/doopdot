...
void Image::new_shrink( size_t newWidth )
{
    if ( newWidth >= m_width )
    {
        return;
    ...
    std::vector<uint8_t> averages( newWidth * m_pixelSize );
    size_t oldRow = 0;
    for ( size_t row = 0; row < m_height; ++row )
    {
        for ( size_t col = 0; col < m_width * m_pixelSize; ++col )
        {
            ...
            averages[ idx ] = getAverage(row, col, 3)[0]; // TODO
        }
